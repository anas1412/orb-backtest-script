...
- SL is anchored at range midpoint: `sl_pct_of_range 0.5` = exact midpoint, `1.0` = opposite boundary. TP = `tp_rr` x risk distance from entry. Optional `sl_move_on_half_tp` ("none" | "breakeven" | "half_risk"): when a candle touches 50% of the TP distance, the SL moves from the NEXT candle on (trigger bar is evaluated against the old stop; conservative). Moved stops land exactly on entry (breakeven) or entry +/- 0.5 x risk (half_risk). Trades record `sl_moved` and `sl_final`.
