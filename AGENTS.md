...
- Frontend state: `lastResult` (backtest payload) and `lastSim` (simulate payload, null when capital disabled) are the single source for all rendering via `renderAll()`. `renderEquityCurve(curveR, curve$, initialCap)` draws $ as the primary axis with cumulative R as a dim dashed right-axis overlay when a sim is present; the chart tooltip shows both. Trade table P&L column and breakdown `$` totals index `sim.pnl` by the SAME index as `lastResult.trades` (both exit-time order).
