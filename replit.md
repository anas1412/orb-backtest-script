...
1. **Generate test data** — click "Generate test data" in the dashboard for synthetic M1 OHLC bars (not real market data).
2. **Upload real data** — upload a CSV with columns `timestamp, open, high, low, close`. Set the "Raw timestamp TZ" field to the source timezone if timestamps are not already UTC.
...
app/
  engine/
    data_loader.py   # CSV loading, timezone normalization, synthetic data generator
